...
DECODE {
		...
		CASE(PUSH_NULL): PUSH(VT_TO_VALUE(VT_NULL));
			LOOP();

		CASE(PUSH_FALSE): PUSH(VT_TO_VALUE(VT_FALSE));
			LOOP();

		CASE(PUSH_TRUE): PUSH(VT_TO_VALUE(VT_TRUE));
			LOOP();
		...
		CASE(LOAD_CONSTANT):
			...
			invokeMethod:
				...
				switch (method->type) {
					...
					case MT_SCRIPT: STORE_CUR_FRAME();
						CreateFrame(vm, curThread, (ObjClosure *)method->obj, argNum);
						LOAD_CUR_FRAME();   //加载最新的frame
						break;

					case MT_FN_CALL: ASSERT(VALUE_IS_OBJCLOSURE(args[0]), "instance must be a closure!");
