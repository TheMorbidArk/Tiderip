    ...
    if (length > MAX_ID_LEN)
    {
        ...
        {   
            ...
            COMPILE_ERROR(vm->curParser,
                "length of identifier \"%s\" should be no more than %d", name, MAX_ID_LEN);
        }
        else
        {   // 编译源码前调用,比如加载核心模块时会调用本函数
            MEM_ERROR("length of identifier \"%s\" should be no more than %d", name, MAX_ID_LEN);
        }
    }
...
Label_IF:
    ...
    else if (matchToken(cu->curParser, TOKEN_ELSE))
    {  //如果有else分支
        //添加跳过else分支的跳转指令
        uint32_t falseBranchEnd = emitInstrWithPlaceholder(cu, OPCODE_JUMP);
...
static void compileClassBody(CompileUnit *cu, Variable classVar)
{
    ...
    {
        if (matchToken(cu->curParser, TOKEN_TIDE))
        {  //处理静态域 "static Tide id"
...
static void compileClassDefinition(CompileUnit *cu)
{
    ...
    if (matchToken(cu->curParser, TOKEN_LESS))
    {   //类继承
